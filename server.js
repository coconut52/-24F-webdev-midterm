...
app.get('/api/items/search', (req, res) => {
  ...
  {
    res.status(404).send('No books found matching the Title');
  }
});
...
app.post('/api/items', (req, res) => {
  ...
  books.push(title);
  res.status(201).json({ message: 'Book added successfuly', books });
...
app.delete('/api/items/:id', (req, res) => {
  ...
  if (bookIndex >= 0 && bookIndex < books.length) {
    ...
    res.status(200).json({ message: 'Book removed successfully', removedBook });
  } else {
    res.status(404).send('Book not found');
  }
});
